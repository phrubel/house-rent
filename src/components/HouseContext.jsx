...
const HouseContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // remove duplicate
    const uniqueCountry = ["locate (any)", ...new Set(allCountries)];
    // set countries
  ...
  const handleSearch = () => {
    setLoading(true);
    // function that checks include str
    const isDefault = (str) => {
    ...
    // get second value of price which is the maximum price and parce it to number
    const maxPrice = parseInt(price.split(" ")[2]);

    const newHouses = housesData.filter((house) => {
      const housePrice = parseInt(house.price);
      // if all valued are selected
      if (
